...
| `pve-host-netmon` | Grafana Alloy network-bandwidth exporter on bare-metal Proxmox host `rabbit-01-psp` | Installs directly on bare metal, not an LXC (unlike `pve-monitoring`); `netdev` collector scoped to physical NICs only, excluding Proxmox bridges/veth/tap interfaces |
