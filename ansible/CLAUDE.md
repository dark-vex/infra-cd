...
| `teleport-server` | Adopts the live Teleport Auth/Proxy config on the OCI `teleport.example.invalid` VM in place | Config/unit are captured from the running box, never authored fresh — see role comments for the break-glass and `--check --diff` rollout requirements |
