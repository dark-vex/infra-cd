resource "proxmox_virtual_environment_vm" "k3s_worker_1" {
  name        = "k3s-worker-1.example.invalid"
  ...
  provisioner "remote-exec" {
    inline = ["sudo hostnamectl set-hostname k3s-worker-1.example.invalid"]
...
resource "proxmox_virtual_environment_vm" "k3s_worker_2" {
  name        = "k3s-worker-2.example.invalid"
  ...
  provisioner "remote-exec" {
    inline = ["sudo hostnamectl set-hostname k3s-worker-2.example.invalid"]
...
resource "proxmox_virtual_environment_vm" "k3s_worker_3" {
  name        = "k3s-worker-3.example.invalid"
  ...
  provisioner "remote-exec" {
    inline = ["sudo hostnamectl set-hostname k3s-worker-3.example.invalid"]
