...
- **Secrets provider by stack** (never hardcode sensitive values — source from the stack's provider(s) below):
  ...
  - **Both:** `terraform/netbox/` (1Password for API credentials; SOPS for on-prem IP/prefix inventory), `terraform/proxmox/rabbit/`, `terraform/proxmox/gozzi-hpelvisor/`, `terraform/proxmox/ec200/`, `terraform/oci/test_vpn/` (1Password for Proxmox/API credentials + SSH keys; SOPS for VM/LXC/instance hostnames), `terraform/aws/` (draft, CODE-20 — not yet applied against real state; 1Password for AWS API credentials, SOPS for bucket names that would otherwise reintroduce a brand string this repo already purged from its history — same rationale as the Proxmox stacks' hostname handling), `terraform/backblaze/` (draft, CODE-20 — not yet applied against real state; 1Password for the B2 application key, SOPS for the endpoint hostname — the 1Password item's hostname field type didn't map to the provider's expected attribute, so it's sourced from SOPS instead; uses the `aminueza/minio` provider, not `hashicorp/aws` — `aws_s3_bucket`'s refresh unconditionally probes CORS/ACL sub-configuration and doesn't handle B2's correctly-formed "not configured" 404 responses gracefully (`hashicorp/terraform-provider-aws#49019`), and `aminueza/minio` documents explicit non-AWS-S3-compatible-backend support via `s3_compat_mode`; CI still fails as of 2026-09-13 on a second, unrelated `aminueza/minio` bug — importing an existing bucket calls `GetBucketPolicy`, which B2 doesn't implement at all, and the import path treats that as fatal. Fixed on `dark-vex/terraform-provider-minio` (branch `fix/import-bucket-policy-not-implemented`, not submitted upstream), verified locally against real B2 via `~/.terraformrc` `dev_overrides` — `terraform plan` comes back clean (`1 to import, 0 to add, 0 to change, 0 to destroy`). `dev_overrides` only affects local runs, not CI, so this stack's CI stays red until the fork's fix is wired into CI some other way)
...
| `terraform-backblaze.yml` | PR/push to `terraform/backblaze/` | Backblaze B2 buckets (draft, CODE-20 — not yet applied against real state) |
