resource "proxmox_virtual_environment_vm" "webserver" {
  name        = "web${count.index + 1}.example.invalid"
  ...
  provisioner "remote-exec" {
    inline = ["sudo hostnamectl set-hostname web${count.index + 1}.example.invalid && wget -O - https://get.ispconfig.org | sudo sh -s -- --i-know-what-i-am-doing"]
