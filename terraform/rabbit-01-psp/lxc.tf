...
resource "proxmox_virtual_environment_container" "haproxy_container" {
  ...
  initialization {
    hostname = "haproxy${count.index + 1}.example.invalid"
  ...
  provisioner "remote-exec" {
    inline = ["sudo hostnamectl set-hostname haproxy${count.index + 1}.example.invalid && sudo apt update && sudo apt install -y haproxy && sudo systemctl enable haproxy && sudo systemctl start haproxy"]
...
resource "proxmox_virtual_environment_container" "proxy_container" {
  ...
  unprivileged = true
  initialization {
    hostname = "squid.example.invalid"
...
resource "proxmox_virtual_environment_container" "satisfactory_container" {
  ...
  initialization {
    hostname = "satisfactory.example.invalid"
...
resource "proxmox_virtual_environment_container" "satisfactory_shared_container" {
  ...
  initialization {
    hostname = "satisfactory-shared.example.invalid"
...
#
#  initialization {
#    hostname = "rtmp${count.index + 1}.example.invalid"
#
#    ip_config {
#      ipv4 {
#        address = "dhcp"
