...
- **`kubenuc-test` and `k3s-prod-test`** are the two legacy classic-bootstrap clusters (auto-discovery, no `apps/kustomization.yaml`). Every app there is either flat auto-discovered files or a nested `Kustomization` CR that cross-references another cluster's real content (see "Add a new application to a cluster" and `kubenuc-test`'s `overlays kubenuc manifests` convention) — there's no clean shape for notifications specifically: a flat copy would apply the `OnePasswordItem` with no `dependsOn` on the (untracked, out-of-band) 1Password operator; a nested `secrets` Kustomization pointed at its own `apps/fluxcd/secrets/` would be double-reconciled by auto-discovery (the exact class of bug fixed in PRs #1697–#1710); and cross-referencing the paired live cluster's whole `apps/fluxcd/` directory drags in resources that don't belong on a test cluster (`kubenuc`'s Nextcloud-specific `betterstack-bridge`, or a `webhook-ingress.yaml` that needs its own `postBuild.substituteFrom` to avoid applying a literal, invalid `${FLUX_WEBHOOK_HOST:=...}` hostname). Needs its own design, not a template copy — `k3s-prod-test`'s existing setup (next bullet) is one such design, using `spec.patches` to strip the drag-in from its own cross-reference; `kubenuc-test` still has none.
- **`k3s-prod-test`'s `apps/fluxcd/deploy.yaml`** intentionally cross-references `clusters/k8s-vms-daniele/apps/fluxcd` — the same `-test`-overlays-prod pairing every other app under `k3s-prod-test/apps/` uses (`teleport-agent`, `blackbox`, `node-exporter`; only `1password` has genuinely independent manifests), not a copy-paste mistake. Unlike those, `fluxcd` has no `manifests/` subdirectory to narrow the reference to — `k8s-vms-daniele/apps/fluxcd/` is a flat directory — so the `flux-notifications` Kustomization pulls in the whole thing and patches it: a JSON6902 op sets `eventMetadata.cluster` to `k3s-prod-test` on both `Alert`s, and three `$patch: delete` entries drop the `github-webhook` `Receiver`, the `flux-webhook` `Ingress` (meaningless on an ephemeral cluster with no real inbound webhook endpoint and an invalid placeholder host), and the nested `slack-secrets` `Kustomization` (would double-reconcile the same path `flux-notifications-secrets` already targets directly — the same dual-ownership bug class fixed elsewhere in PRs #1697–#1710). `fluxcd` still stays in `validate-k8s-vms.yml`'s `EXCLUDED_APPS`, for an unrelated reason (its real shared Slack webhook), so this app remains dormant/uncommitted-to-CI regardless of this fix.
