...
5. If the cluster's `apps/` has an explicit `apps/kustomization.yaml` (currently `oc-ampere`, `k3s-rabbit`; `kubenuc`/`k8s-vms-daniele` pending), add the new app's nested Kustomization file (or its directory, if it has its own wrapper `kustomization.yaml`) to that `resources:` list — otherwise it silently never gets applied. Where no `apps/kustomization.yaml` exists yet, Flux/kustomize auto-discovers all YAML files in a path.
...
- **Explicit `apps/kustomization.yaml`** (listing every app's nested Kustomization file, or its directory for apps with their own wrapper `kustomization.yaml`, instead of relying on kustomize-controller's recursive auto-discovery) is now in place on `oc-ampere` and `k3s-rabbit` — this closes a dual-ownership bug where an app's live objects were reconciled by both the flattened `apps` Kustomization and its own nested child Kustomization (hit in practice on `oc-ampere`'s and `k8s-vms-daniele`'s `system-upgrade-controller` `Plan` CRs). `kubenuc` (29 app dirs) and `k8s-vms-daniele` (18) have the same latent dual-ownership issue but are **not yet fixed** — larger per-cluster enumeration, tracked as a follow-up. See "Add a new application to a cluster" above: any new app on a cluster with an explicit `apps/kustomization.yaml` must be added to its `resources:` list.
