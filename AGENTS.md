...
This file provides context for AI assistants (Codex and similar) working in this repository. It covers the codebase structure, development workflows, conventions, and key rules to follow.
...
├── .Codex/                # Codex configuration and permissions
...
docs: update AGENTS.md with cluster conventions
```
...
Codex uses a layered approach for domain-specific operations in this repository:
...
**Hybrid model strategy:** Use Ollama (local) for routine boilerplate generation (YAML manifests, HCL resources). Use Codex API for complex reasoning (dependency analysis, security review, multi-file refactors).
